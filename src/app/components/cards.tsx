...
export default function Cards(){
    ...
    return (
        ...
        {listProject?.map((project) => (
            <li key={project.id} className="flex flex-col justify-between sm:w-full lg:w-[400px] md:h-[200px] lg:h-74 p-6 border rounded-md shadow bg-gray-800 border-gray-700">
                <div>
                    <h5 className="mb-2 text-2xl font-bold tracking-tight text-white break-all">{project.name}</h5>
                    <p className="mb-3 font-normal text-gray-400 break-words">{project.description}</p>
                    <div className="flex space-x-2">
                    <Link href={project.html_url} target="_blank" className="flex hover:text-rose-800 text-white ">
                    ...
                    <Link href={project.homepage} target="_blank" className="flex hover:text-rose-800 text-white ">
